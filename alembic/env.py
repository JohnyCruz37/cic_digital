...
def _ensure_content_schema(connection) -> None:
    """Alembic version table lives in `content`; schema must exist before first run."""
    connection.execute(text(f"CREATE SCHEMA IF NOT EXISTS {CONTENT_SCHEMA}"))
    connection.commit()
...
def run_migrations_online() -> None:
    ...
    with connectable.connect() as connection:
        _ensure_content_schema(connection)
        context.configure(
